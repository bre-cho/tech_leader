"""HTTP contract tests for Adobe and Canva production adapters.

Adapters now use httpx.AsyncClient internally (with asyncio.sleep for polling)
so these tests patch httpx.AsyncClient with an async-compatible fake.
"""
...
class _FakeAsyncHttpClient:
    """Async context manager that simulates httpx.AsyncClient."""
...
def _no_sleep(_):
    """Coroutine that immediately returns so asyncio.sleep is a no-op in tests."""
    async def _inner():
        pass
    return _inner()
...
def test_adobe_production_submit_and_poll_success(monkeypatch):
    ...
    monkeypatch.setattr(adobe_module.httpx, "AsyncClient", factory)
    monkeypatch.setattr(adobe_module.asyncio, "sleep", _no_sleep)
