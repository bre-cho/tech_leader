...
async def process_render_dispatch(db: Session, job_id: str) -> None:
    ...
    for scene in scenes_to_dispatch:
        # Circuit breaker pre-check: skip scenes whose provider circuit is open
        # to avoid hammering a failing provider and inflating error counts.
        try:
            from app.services.provider_routing.circuit_breaker import get_breaker  # noqa: PLC0415
            provider_key = normalize_provider_name(scene.provider)
            ...
            if breaker.is_open():
                transition_scene_to_failed(
                    ...
                    error_message=(
                        f"Circuit breaker OPEN for provider={provider_key}; "
                        "scene skipped to protect provider from further load. "
                        "The circuit will reset automatically after the cool-down period."
                    ),
                    failure_code="CIRCUIT_BREAKER_OPEN",
                    failure_category="provider_circuit_breaker",
                    raw_response=None,
                    source="dispatch",
                    final_status="failed",
                )
            ...
            pass  # Fail open: if CB check fails, proceed with dispatch
