...
async def poll_scene_task(
    *,
    ...
    if not provider_operation_name and not provider_task_id:
        return NormalizedStatusResult(
            provider=normalized_provider,
            state="failed",
            error_message=f"Missing both provider_operation_name and provider_task_id for {normalized_provider} poll",
            failure_code="MISSING_PROVIDER_REFS",
