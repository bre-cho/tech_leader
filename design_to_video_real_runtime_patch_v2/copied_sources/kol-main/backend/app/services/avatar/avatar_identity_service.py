...
_AVATAR_IDENTITY_NO_CANONICAL_TOTAL = (
    _PromCounter(
        "avatar_identity_no_canonical_total",
        "Count of avatar identity checks that had no canonical reference embedding.",
    )
...
# ---------------------------------------------------------------------------
# Configurable via IDENTITY_GATE_THRESHOLD env var (default 0.80).
IDENTITY_GATE_THRESHOLD: float = float(os.getenv("IDENTITY_GATE_THRESHOLD", "0.80"))
...
class AvatarConsistencyGuard:
    ...
    def lock_frame_sequence(
        ...
        if canonical is None:
            if _AVATAR_IDENTITY_NO_CANONICAL_TOTAL is not None:
                _AVATAR_IDENTITY_NO_CANONICAL_TOTAL.inc()
            logger.warning(
                "Avatar identity canonical embedding missing; fallback sim=1.0 for avatar_id=%s",
                avatar_id,
            )
...
class AvatarIdentityService:
    ...
    def upsert_visual(self, db: Session, avatar_id: str, data: dict) -> AvatarVisualDna:
        ...
        if source_media and "embedding_vector" not in data:
            try:
                ...
                logger.warning(
                    "upsert_visual: embedding extraction failed for avatar_id=%s source=%s: %s",
                    avatar_id, source_media, exc,
                )
        return visual
    ...
    def get_identity_vector(self, db: Session, avatar_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "get_identity_vector: reference frame lookup failed for avatar_id=%s: %s",
                avatar_id, exc,
            )
        if voice:
    ...
    def get_reference_frames(
        ...
        if db is not None:
            try:
                ...
                logger.warning(
                    "get_reference_frames: DB lookup failed for avatar_id=%s: %s",
                    avatar_id, exc,
                )
        return [
    ...
    def verify_render_output(
        ...
        if consistency_score < IDENTITY_GATE_THRESHOLD:
            ...
            try:
                ...
                logger.warning(
                    "verify_render_output: drift tracking failed for avatar_id=%s: %s",
                    avatar_id, exc,
                )
            ...
            try:
                ...
                logger.warning(
                    "verify_render_output: success tracking failed for avatar_id=%s: %s",
                    avatar_id, exc,
                )
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "refresh_reference_frame: rollback failed for avatar_id=%s: %s",
                    avatar_id, rb_exc,
                )
