...
def get_or_create_worker_override(db: Session, *, queue_name: str = DEFAULT_QUEUE_NAME) -> WorkerConcurrencyOverride:
    ...
    try:
        ...
        # Another worker inserted the row concurrently (UNIQUE queue_name constraint).
        # Roll back and fetch the winner row.
        db.rollback()
        existing = db.query(WorkerConcurrencyOverride).filter(WorkerConcurrencyOverride.queue_name == queue_name).first()
        if existing:
            return existing
        raise
