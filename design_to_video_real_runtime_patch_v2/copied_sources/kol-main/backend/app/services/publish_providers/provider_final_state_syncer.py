...
class ProviderStatePollOrchestrator:
    ...
    def poll_until_stable(
        ...
        Args:
            ...
            _sleep: When True, sleep between rounds (use only in synchronous
                workers). The sleep is event-loop-aware: when called from inside
                a running asyncio event loop ``asyncio.sleep`` is used via
                ``run_coroutine_threadsafe`` to avoid blocking the loop thread.
        ...
        def _on_task_done(task: "_asyncio.Task[None]", future: "_cf.Future[None]") -> None:  # type: ignore[name-defined]
            """Callback to resolve *future* when the asyncio sleep task finishes."""
            exc = task.exception()
            if exc is not None:
                if not future.done():
        ...
        def _do_sleep(secs: float) -> None:
            """Sleep for *secs* without blocking a running asyncio event loop."""
            ...
            if loop is not None and loop.is_running():
                future: _cf.Future[None] = _cf.Future()
                coro = _asyncio.sleep(secs)
                task = loop.create_task(coro)
                task.add_done_callback(lambda t: _on_task_done(t, future))
                future.result(timeout=secs + 5)
            else:
        ...
        for round_idx in range(1, self._max_rounds + 1):
            if _sleep and round_idx > 1:
                _do_sleep(self._poll_interval_sec * (2 ** (round_idx - 2)))
