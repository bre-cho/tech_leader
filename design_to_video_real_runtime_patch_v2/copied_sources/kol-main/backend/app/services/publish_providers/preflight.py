...
from app.services.avatar.render_quality_gate import PUBLISH_QUALITY_THRESHOLD
        ...
        if quality_report is not None and not quality_report.passed:
            errors.append(
                (
                    ...
                    f"composite_score={quality_report.composite_score:.3f} "
                    f"< threshold={PUBLISH_QUALITY_THRESHOLD:.3f}"
                )
            )
