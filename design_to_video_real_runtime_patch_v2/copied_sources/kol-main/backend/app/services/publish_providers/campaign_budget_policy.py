    ...
    def _count_today(
        ...
        *,
        ...
        try:
            ...
            logger.error("CampaignBudgetPolicy count query failed; failing open: %s", exc, exc_info=True)
        ...
        for c in campaigns:
            ...
            if performance_model is not None:
                try:
                    ...
                    logger.warning(
                        "PortfolioBudgetOrchestrator: performance_model lookup failed for campaign_id=%s: %s",
                        cid,
                        exc,
                    )
