...
async def run_narration_job(db: Session, narration_job_id: str) -> NarrationJob:
    ...
    if len(segment_paths) == 1:
        # Single segment — copy directly to avoid an unnecessary FFmpeg round-trip.
        import shutil
        shutil.copy2(segment_paths[0], str(final_path))
    elif segment_paths:
        # Use FFmpeg concat demuxer so that the resulting file contains valid MP3
        # frames.  Raw byte-concatenation of separate MP3 streams is invalid and
        # produces files that most decoders reject or play back incorrectly.
