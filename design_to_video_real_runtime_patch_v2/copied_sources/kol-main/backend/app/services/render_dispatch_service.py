...
def _safe_json_loads(value: str | None) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            "render_dispatch_service: JSON decode error; returning empty payload. "
...
def _build_dispatch_idempotency_key(provider: str, raw: dict[str, Any]) -> str:
    basis = {
        "provider": provider,
        "project_id": raw.get("project_id"),
        "job_id": raw.get("job_id"),
        "scene_index": raw.get("scene_index"),
        "prompt": _resolve_prompt_text(raw),
        "duration_seconds": raw.get("duration_seconds") or raw.get("provider_target_duration_sec"),
        "seed": raw.get("seed"),
    }
    data = json.dumps(basis, ensure_ascii=False, sort_keys=True, default=str)
    return f"{provider}:" + hashlib.sha256(data.encode("utf-8")).hexdigest()
...
def _build_provider_payload(raw: dict[str, Any], provider: str) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "idempotency_key": raw.get("idempotency_key") or _build_dispatch_idempotency_key(provider, raw),
    }
...
async def dispatch_scene_task(provider: str, request_payload_json: str) -> NormalizedSubmitResult:
    ...
    with SessionLocal() as db:
        assert_provider_quota_available(db, provider=normalized_provider)
