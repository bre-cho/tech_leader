...
class ProviderIngressSignatureError(ValueError):
    """Raised when a provider ingress callback has an invalid or missing signature.

    Attributes
    ----------
    reason:
        Human-readable description of the verification failure (e.g.
        ``"missing_secret"``, ``"timestamp_expired"``, ``"digest_mismatch"``).
    """

    def __init__(self, reason: str = "invalid_signature") -> None:
        self.reason = reason
        super().__init__(f"Provider ingress signature verification failed: {reason}")
