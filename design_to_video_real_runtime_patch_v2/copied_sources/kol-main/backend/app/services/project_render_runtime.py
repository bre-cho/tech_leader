...
def get_project_render_status(db: Session, project_id: str) -> dict:
    ...
    if data.status == "completed":
        ...
        _maybe_enqueue_publish_job(db, project, data)
    save_project(project)
...
def _maybe_enqueue_publish_job(db: Session, project: dict, render_data: object) -> None:
    # Idempotent bridge: once queued, subsequent status polls must not create duplicate jobs.
    if project.get("publish_queue_created"):
        return
    ...
    final_video_url = getattr(render_data, "final_video_url", None) or getattr(render_data, "output_url", None)
    ...
    try:
        ...
        _log.warning("Publish scheduler unavailable; skip auto publish bridge: %s", exc)
        return

    plan_item = {
        "day_index": 1,
        "title": project.get("title") or f"Project {project_id}",
        "description": project.get("description") or "Auto queued from render completion",
        "content_goal": project.get("goal") or "engagement",
        "format": project.get("format") or "9:16",
        "video_url": final_video_url,
        "thumbnail_url": project.get("thumbnail_url") or getattr(render_data, "thumbnail_url", None),
        "metadata": {
            "project_id": project_id,
            "render_job_id": project.get("render_job_id"),
            "market_code": project.get("market_code"),
            "avatar_id": project.get("avatar_id"),
        },
    }

    platform = project.get("publish_platform") or "shorts"
    jobs = publish_scheduler.create_publish_jobs(
        db=db,
        channel_plan_id=project_id,
        plan_items=[plan_item],
        platform=platform,
    )

    project["publish_queue_created"] = True
    project["publish_job_ids"] = [job.id for job in jobs]
