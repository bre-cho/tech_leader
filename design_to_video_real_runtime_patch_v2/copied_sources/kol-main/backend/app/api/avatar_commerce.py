...
def record_attribution_as_performance(
    ...
    for touch in attribution.get("attributions", []):
        try:
            ...
            _logger.warning(
                "record_attribution_as_performance: failed to record touch for campaign=%s index=%s: %s",
                campaign_id,
                records_written,
                exc,
            )
...
def intelligence_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        _logger.warning("intelligence_status: failed to count PerformanceRecord: %s", exc)

    variant_count = 0
    try:
        variant_count = db.query(VariantRunRecord).count()
    except Exception as exc:  # noqa: BLE001
        _logger.warning("intelligence_status: failed to count VariantRunRecord: %s", exc)

    objection_count = 0
    try:
        objection_count = db.query(ObjectionPattern).filter(ObjectionPattern.is_active.is_(True)).count()
    except Exception as exc:  # noqa: BLE001
        _logger.warning("intelligence_status: failed to count ObjectionPattern: %s", exc)
    ...
    try:
        ...
        _logger.warning("intelligence_status: failed to query ScoringCalibration: %s", exc)
