...
import { isRuntimeMockAllowed } from "@/lib/config/runtime-guards";
    ...
    if (!process.env.OPENAI_API_KEY) {
      if (!isRuntimeMockAllowed()) {
        return Response.json(
          {
            error:
              "Service unavailable: OPENAI_API_KEY is not configured and runtime mock is disabled in production-like environment."
          },
          { status: 503 }
        );
      }
      observability.recordFallbackActivation("api/image/generate", "runtime-mock");
